...
class BaseEngine(object):
    ...
    def detect_video(self):
        ...
        while not rospy.is_shutdown():
            ...
            width = int(640)
            height = int(480)
            out_yolo = cv2.VideoWriter('/home/nvidia/xycar_ws/src/yolov7/src/results_yolo2.avi',fourcc,30,(width,height))
            out_bev = cv2.VideoWriter('/home/nvidia/xycar_ws/src/yolov7/src/results_bev2.avi',fourcc,30,(540,540))
            bev = np.zeros((540, 540, 3), dtype=np.uint8)
            fps = 0
            import time
            while True:
                # ret, frame = cap.read()
                ...
                if dets is not None:
                    ...
                    frame, bev, new_box, new_score, new_ids, new_depth = self.vis(frame, final_boxes, final_scores, final_cls_inds,
                                    conf=0.8, class_names=self.class_names)
                  
                    self.publisher(new_box, new_score, new_ids, new_depth)

                
                cv2.imshow('frame', frame)
                cv2.imshow('birdeye', bev)
                # cv2.waitKey(1)
    ...
    def publisher(self, boxes, confs, classes,depth):
        ...
        if len(depth) and min(depth) < self.depth_thres:
            idx = depth.index(min(depth))
            self._write_message(detection_results, boxes[idx], confs[idx], classes[idx],depth[idx])
            self.detection_pub.publish(detection_results)
        else:
            # self._write_message(detection_results, boxes, confs, classes,depth)
            self.detection_pub.publish(BoundingBoxes())
...
# def preproc(image, input_size, mean, std, swap=(2, 1, 0)):
    
#     img = np.array(image, dtype = np.uint8)
#     new_resolution = (640,640)
#     image_raw = cv2.resize(img, new_resolution, interpolation=cv2.INTER_LINEAR)
#     # image_raw = cv2.cvtColor(image_raw, cv2.COLOR_BGR2RGB)
#     # plt.imshow(image_raw)
#     # plt.show()
#     # 640,640,3 -> 3 640 640 
    
#     image = np.transpose(np.array(image_raw, dtype=float) / 255,(2,0,1))
    
#     # bgr -> rgb 201
#     # image[0] = image[0].transpose(swap)
#     image = np.expand_dims(image, axis=0)
#     image = np.array(image, dtype=np.float32, order="C")

#     # image_resized = cv2.resize(img, new_resolution, interpolation=cv2.INTER_LINEAR)
#     # image_resized = np.array(image_raw.resize(new_resolution, resample=Image.BILINEAR), dtype=np.uint8)
        
#     # print(img.shape)
#     r = min(input_size[1] / img.shape[1], input_size[0] / img.shape[0])
#     # resized_img = cv2.resize(
#     #     img,
#     #     (int(img.shape[0] * r), int(img.shape[1] * r)),
#     #     interpolation=cv2.INTER_LINEAR,
#     # ).astype(np.float32)
#     # padded_img[: int(img.shape[1] * r), : int(img.shape[0] * r)] = resized_img
#     # # if use yolox set
#     # # padded_img = padded_img[:, :, ::-1]
#     # # padded_img /= 255.0
#     # padded_img = padded_img[:, :, ::-1]
